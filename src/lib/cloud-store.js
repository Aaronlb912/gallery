const FILE = 'gallery-galleries.json'
...
export async function cloudSignIn() {
  ...
  try {
    ...
    if (/closed|cancel|abort|denied|dismiss/i.test(msg)) {
...
export async function pullCloudWorkspace() {
  try {
    ...
    return { ok: false, error: 'Could not load your saved galleries.' }
  }
}
...
export async function pushCloudWorkspace(workspace) {
  try {
    ...
    return { ok: false, error: 'Could not save your galleries.' }
  }
}
